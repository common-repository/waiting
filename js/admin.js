...
PBC.down.adminit = function(down, id){
	...
	$('.pbc-date-picker').datepicker({
		isRTL: waiting_i18n.ir_rtl,
		dateFormat: waiting_i18n.date_format
	});		
...
$('body').on('click', '#pbc-edit-style', function(e){
	$('.pbc-choose-style').attr('id', 'pbc-choose-style').addClass('button-primary').text('Change');
...
function prepareForSaving( form ){
	var raw = rawValues( $(form).find('input.wpb-raw') ),
		to = $('#cd_to input[name=to_date]').datepicker('getDate'),
		start = $('#cd_from input[name=start_date]').datepicker('getDate');
	
	if( to instanceof Date ){
		to = +new Date( to.getYear()+1900, to.getMonth(), to.getDate(), raw.to_hours, raw.to_mins, raw.to_secs);
	}
	start = +new Date( start.getYear()+1900, start.getMonth(), start.getDate(), raw.start_hours, raw.start_mins, raw.start_secs);
	...
	PBC.down.data.style.shorten_label = raw.shorten_label;
	PBC.down.data.style.lowercase_label = raw.lowercase_label;
...
$('body').on('submit', '#pbc-form', function(e){
	e.preventDefault();
	var is_new = prepareForSaving( this ); //return;
...
$('body').on('submit', '#pbc-other-settings', function(e){
	e.preventDefault(); var fo = $(this);
	var fields = rawValues( fo.find('input') );
	fo.find('.button').text('Saving....');
	PBC.ajax({'action': 'pbc_save_other_settings', 'pbc_clean_on_uninstall': fields.clean_on_uninstall }, function(re){
		fo.find('.button').text('Save');
	});
});
...
PBC.downs.deploy = function( downs ){
	...
	downs.forEach(function(down){
		if( !down.meta.units ) down.meta.units = [];
	...
	$('body').on('click', 'tr .pbc-copy', function(e){
		var down = JSON.parse( JSON.stringify(PBC.downs[ $(this).data('down') ]));
			down.meta.name += '-'+(String(Math.random()).replace('.', '').substr(0, 6));
			down.meta.id = 'nw';
		PBC.down.adminit( down, 'nw' );
	});
...
PBC.startDateTemplate = function( oto, offset ){
	return PBC.template('#pbc-countdown-start-date-tmpl')({'ostart': PBC.date( new Date(oto), offset )});
};
...
PBC.date = function(d, offset){
	offset = offset || 0;
	//console.log(d.getTimezoneOffset() , parseFloat(offset), ((d.getTimezoneOffset() - parseFloat(offset)) * 60 * 1000));
	...
	var d_o = {'y': d.getFullYear(), 'mo': (d.getMonth()+1), 'd': d.getDate(), 'h':d.getHours(), 'mi':d.getMinutes(), 's':d.getSeconds()};
		...
		//d_o.date = d_o.y + '-' + d_o.mo + '-' + d_o.d ; //d_o.mo + '/' + d_o.d + '/' + d_o.y;
		//d_o.date = jQuery.datepicker.formatDate(waiting_i18n.date_format, new Date(d_o.y + '-' + d_o.mo + '-' + d_o.d));
		d_o.date = jQuery.datepicker.formatDate(waiting_i18n.date_format, d);
...
function pbcFontLink(){
	if(PBC.down.data.style.css.unit[3] !== 'inherit')
		$('#pbc-font-link').attr('href', '//fonts.googleapis.com/css?family='+PBC.down.data.style.css.unit[3]);
...
function customServicesFunctions(){
	$('#pb-c-s-country').text(['N', 'e', 'p', 'a', 'l'].join(''));
	$('#pb-custom-services > div > p').on('click', function(e){
		$(this).children('i').toggleClass('dashicons-arrow-down-alt2 dashicons-arrow-up-alt2');
