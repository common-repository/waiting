...
/*
	Plugin Name: Waiting
	Plugin URI: https://plugin.builders/waiting/?from=plugins
...
class WPB_Waiting{
	...
	public function createMenu(){
		add_menu_page(
			'Waiting',
			'Waiting',
			apply_filters('waiting_page_roles', 'manage_options'),
			'waiting',
		...
		if($id === 'nw'){
			...
			$sql = "UPDATE ".$this->table."
					SET 
						...
						`extras` = %s 
					WHERE id = ".$id;
			//echo $wpdb->prepare( $sql, array($down['meta']['name'], maybe_serialize($down), $html['offertext'], $html['replacetext'], maybe_serialize( $extras ) ) );
	...
	**/
	...
	public function translateTerms(){
		...
		// Write English term for each unit below.
		
		$English = array(
			'years' => 'English for years',
			'months' => '',
			'weeks' => '',
			'days' => '',
			'hours' => '',
		...
		// Write Thai term for each unit below.
		
		$Thai = array(
			'years' => 'Thai for years',
			'months' => '',
			'weeks' => '',
			'days' => '',
			'hours' => '',
		...
		self::$terms['units'] = ( strpos($url, 'thai') === false ? $English : $Thai );
		*/
